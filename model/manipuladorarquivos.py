...
class ManipulaArquivos:
    ...
    def salvar_dados_em_json(self,lista_clientes):
    ...
    #######################################################################
        ...
        while no_atual is not None:# Transformar um objecto cliente em um dicionario
            ...
            dados_clientes = {
                ...
                'orçamento':no_atual.get_element().get_orcamento(),
                'despesas':[]
            }
            ...
            while no_atual_despesas is not None:
                dados_despesas = {
                    ...
                    'descrição':no_atual_despesas.get_element().get_descricaoDespesa(),
            ...
            while no_clientes_existentes is not None:
                if no_clientes_existentes.element.get_nome()==no_atual.get_element().get_nome() and no_clientes_existentes.element.get_nif()==no_atual.get_element().get_nif():
                    clinte_existe = no_clientes_existentes.element.get_element()
                    break
            ...
            if clinte_existe is not None:
                ...
                'nome':clinte_existe.get_element().get_nome(),
                'nif':clinte_existe.get_element().get_nif(),
                'password':clinte_existe.get_element().get_password(),
                'orçamento':clinte_existe.get_element().get_orcamento(),
                'despesas':[]
                }

                clinte_existe_no_despesas = clinte_existe.get_element().get_listaDespesas().head
                ...
                while clinte_existe_no_despesas is not None:
                    ...
                    'descrição':clinte_existe_no_despesas.get_element().get_descricaoDespesa(),
               ...
               if conteudo.strip():  # Verifica se o conteúdo não está vazio
                 ...
                 if 'lista de clientes' in dados and dados['lista de clientes']:
                    for dados_clientes in dados['lista de clientes']:
                       ...
                       cliente.set_orcamento(dados_clientes['orçamento'])

                       for dados_despesa in dados_clientes['despesas']:
                          cliente.adicionar_despesa(dados_despesa['data'],dados_despesa['categoria'],dados_despesa['valor'],dados_despesa['descrição'])
